...
class GTSRBDataset(Dataset):
    """Class that read the GTSRB dataset for traffic signs classification."""

    def __init__(self, csv_file, root_dir, transform=None):
        """
        Args:
            csv_file (string): Path to the csv file with annotations.
            root_dir (string): Main directory with images.
            transform (callable, optional): Optional transform to be applied
                on a sample.
        """
        self.gtsrb = pd.read_csv(csv_file).sample(frac=1)
        self.root_dir = root_dir
        self.transform = transform
        self.gtsrb["ClassId"][self.gtsrb["ClassId"]==13] = 0
        self.targets = self.gtsrb["ClassId"]
    ...
    def __getitem__(self, idx):
        ...
        img_name = os.path.join(self.root_dir,
                                    self.gtsrb["Path"].iloc[idx])
        image = io.imread(img_name)
        labels = self.gtsrb["ClassId"].iloc[idx]
...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        if new_h > new_w:
            ...
            img = np.pad(img, ((0, 0),(required_pad1, required_pad2), (0, 0)),
            ...
            required_pad1 = int(required_pad1)
            img = np.pad(img, ((required_pad1, required_pad2),(0, 0), (0, 0)),
                         'constant', constant_values=(0))
