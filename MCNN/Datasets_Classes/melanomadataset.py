    ...
    def __init__(self, csv_file, root_dir, transform=None):
        ...
        Args:
            ...
            transform (callable, optional): Optional transform to be applied
                on a sample.
        """
        self.isic = pd.read_csv(csv_file).sample(frac=1) # when used in colab
        self.root_dir = root_dir
        self.transform = transform
        self.targets = self.isic["target"]

    def __len__(self):
        return len(self.isic)

    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()

        self.isic = self.isic.sample(frac=1)
        img_name = os.path.join(self.root_dir,
                                self.isic["image_name"].iloc[idx])
        image = io.imread(img_name)
        labels = self.isic["target"].iloc[idx]
